...
static unsigned hdr_sample_rate_hz(const uint8_t *h)
{
    ...
    return g_hz[(((h[2]) >> 2) & 3)] >> (int)!((h[1]) & 0x8) >> (int)!((h[1]) & 0x10);
}

static unsigned hdr_frame_samples(const uint8_t *h)
{
    return ((h[1] & 6) == 6) ? 384 : (1152 >> (int)((h[1] & 14) == 2));
}
...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t *hdr, L12_scale_info *sci)
{
    ...
    int mode = (((hdr[3]) >> 6) & 3);
    int nbands, stereo_bands = (mode == 3) ? 0 : (mode == 1) ? ((((hdr[3]) >> 4) & 3) << 2) + 4 : 32;
    ...
    } else
    {
        ...
        int sample_rate_idx = (((hdr[2]) >> 2) & 3);
        unsigned kbps = hdr_bitrate_kbps(hdr) >> (int)(mode != 3);
        if (!kbps)
        {
            kbps = 192;
        }
    ...
    sci->stereo_bands = (uint8_t)((stereo_bands) > (nbands) ? (nbands) : (stereo_bands));
...
{
    int i, k;
    __builtin___memcpy_chk (dst + 576 + sci->stereo_bands*18, dst + sci->stereo_bands*18, (sci->total_bands - sci->stereo_bands)*18*sizeof(float), __builtin_object_size (dst + 576 + sci->stereo_bands*18, 0));
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    int sr_idx = ((((hdr[2]) >> 2) & 3) + (((hdr[1] >> 3) & 1) + ((hdr[1] >> 4) & 1))*3); sr_idx -= (sr_idx != 0);
    ...
    do
    {
        ...
        gr->scalefac_compress = (uint16_t)get_bits(bs, ((hdr[1]) & 0x8) ? 4 : 9);
        ...
        gr->preflag = ((hdr[1]) & 0x8) ? get_bits(bs, 1) : (gr->scalefac_compress >= 500);
...
static void L3_read_scalefactors(uint8_t *scf, uint8_t *ist_pos, const uint8_t *scf_size, const uint8_t *scf_count, bs_t *bitbuf, int scfsi)
{
    ...
    for (i = 0; i < 4 && scf_count[i]; i++, scfsi *= 2)
    {
        ...
        if (scfsi & 8)
        {
            __builtin___memcpy_chk (scf, ist_pos, cnt, __builtin_object_size (scf, 0));
        } else
        {
            int bits = scf_size[i];
            if (!bits)
            {
                __builtin___memset_chk (scf, 0, cnt, __builtin_object_size (scf, 0));
                __builtin___memset_chk (ist_pos, 0, cnt, __builtin_object_size (ist_pos, 0));
            } else
            {
...
static void L3_decode_scalefactors(const uint8_t *hdr, uint8_t *ist_pos, bs_t *bs, const L3_gr_info_t *gr, float *scf, int ch)
{
    ...
    } else
    {
        ...
        int k, modprod, sfc, ist = ((hdr[3]) & 0x10) && ch;
    ...
    gain_exp = gr->global_gain + -1*4 - 210 - ((((hdr[3]) & 0xE0) == 0x60) ? 2 : 0);
    gain = L3_ldexp_q2(1 << ((((255 + -1*4 - 210) + 3) & ~3)/4), (((255 + -1*4 - 210) + 3) & ~3) - gain_exp);
...
static void L3_huffman(float *dst, bs_t *bs, const L3_gr_info_t *gr_info, const float *scf, int layer3gr_limit)
{
            ...
            do
            {
                ...
                for (j = 0; j < 2; j++, dst++, leaf >>= 4)
                {
                    ...
                    if (lsb == 15 && linbits)
                    {
                        ...
                        while (bs_sh >= 0) { bs_cache |= (uint32_t)*bs_next_ptr++ << bs_sh; bs_sh -= 8; };
    ...
    for (np = 1 - big_val_cnt;; dst += 4)
    {
        ...
        if (leaf & (128 >> 0)) { dst[0] = ((int32_t)bs_cache < 0) ? -one : one; { bs_cache <<= (1); bs_sh += (1); } };
        if (leaf & (128 >> 1)) { dst[1] = ((int32_t)bs_cache < 0) ? -one : one; { bs_cache <<= (1); bs_sh += (1); } };
        if (!--np) { np = *sfb++/2; if (!np) break; one = *scf++; };
        if (leaf & (128 >> 2)) { dst[2] = ((int32_t)bs_cache < 0) ? -one : one; { bs_cache <<= (1); bs_sh += (1); } };
        if (leaf & (128 >> 3)) { dst[3] = ((int32_t)bs_cache < 0) ? -one : one; { bs_cache <<= (1); bs_sh += (1); } };
        while (bs_sh >= 0) { bs_cache |= (uint32_t)*bs_next_ptr++ << bs_sh; bs_sh -= 8; };
    }
...
static void L3_stereo_process(float *left, const uint8_t *ist_pos, const uint8_t *sfb, const uint8_t *hdr, int max_band[3], int mpeg2_sh)
{
    ...
    unsigned i, max_pos = ((hdr[1]) & 0x8) ? 7 : 64;
    ...
    for (i = 0; sfb[i]; i++)
    {
        ...
        {
            float kl, kr, s = ((hdr[3]) & 0x20) ? 1.41421356f : 1;
            if (((hdr[1]) & 0x8))
            {
                kl = g_pan[2*ipos];
                kr = g_pan[2*ipos + 1];
...
static void L3_intensity_stereo(float *left, uint8_t *ist_pos, const L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    if (gr->n_long_sfb)
    {
        max_band[0] = max_band[1] = max_band[2] = ((((max_band[0]) < (max_band[1]) ? (max_band[1]) : (max_band[0]))) < (max_band[2]) ? (max_band[2]) : (((max_band[0]) < (max_band[1]) ? (max_band[1]) : (max_band[0]))));
    }
    for (i = 0; i < max_blocks; i++)
    {
        int default_pos = ((hdr[1]) & 0x8) ? 3 : 0;
...
static void L3_reorder(float *grbuf, float *scratch, const uint8_t *sfb)
{
    ...
    __builtin___memcpy_chk (grbuf, scratch, (dst - scratch)*sizeof(float), __builtin_object_size (grbuf, 0));
}
...
static void L3_imdct_gr(float *grbuf, float *overlap, unsigned block_type, unsigned n_long_bands)
{
    ...
    if (block_type == 2)
        ...
        L3_imdct36(grbuf, overlap, g_mdct_window[block_type == 3], 32 - n_long_bands);
}
...
static void L3_save_reservoir(mp3dec_t *h, mp3dec_scratch_t *s)
{
    ...
    if (remains > 0)
    {
        __builtin___memmove_chk (h->reserv_buf, s->maindata + pos, remains, __builtin_object_size (h->reserv_buf, 0));
    }
    h->reserv = remains;
}
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs, mp3dec_scratch_t *s, int main_data_begin)
{
    ...
    int bytes_have = ((h->reserv) > (main_data_begin) ? (main_data_begin) : (h->reserv));
    __builtin___memcpy_chk (s->maindata, h->reserv_buf + ((0) < (h->reserv - main_data_begin) ? (h->reserv - main_data_begin) : (0)), ((h->reserv) > (main_data_begin) ? (main_data_begin) : (h->reserv)), __builtin_object_size (s->maindata, 0));
    __builtin___memcpy_chk (s->maindata + bytes_have, bs->buf + bs->pos/8, frame_bytes, __builtin_object_size (s->maindata + bytes_have, 0));
...
static void L3_decode(mp3dec_t *h, mp3dec_scratch_t *s, L3_gr_info_t *gr_info, int nch)
{
    ...
    {
        int aa_bands = 31;
        int n_long_bands = (gr_info->mixed_block_flag ? 2 : 0) << (int)(((((h->header[2]) >> 2) & 3) + (((h->header[1] >> 3) & 1) + ((h->header[1] >> 4) & 1))*3) == 2);
...
static void mp3d_synth(float *xl, mp3d_sample_t *dstl, int nch, float *lins)
{
    ...
    for (i = 14; i >= 0; i--)
    {
        ...
        { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 0*64]; float *vy = &zlin[4*i - (15 - 0)*64];; for (j = 0; j < 4; j++) b[j] = vz[j]*w1 + vy[j]*w0, a[j] = vz[j]*w0 - vy[j]*w1; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 1*64]; float *vy = &zlin[4*i - (15 - 1)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vy[j]*w1 - vz[j]*w0; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 2*64]; float *vy = &zlin[4*i - (15 - 2)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vz[j]*w0 - vy[j]*w1; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 3*64]; float *vy = &zlin[4*i - (15 - 3)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vy[j]*w1 - vz[j]*w0; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 4*64]; float *vy = &zlin[4*i - (15 - 4)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vz[j]*w0 - vy[j]*w1; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 5*64]; float *vy = &zlin[4*i - (15 - 5)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vy[j]*w1 - vz[j]*w0; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 6*64]; float *vy = &zlin[4*i - (15 - 6)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vz[j]*w0 - vy[j]*w1; } { int j; float w0 = *w++; float w1 = *w++; float *vz = &zlin[4*i - 7*64]; float *vy = &zlin[4*i - (15 - 7)*64];; for (j = 0; j < 4; j++) b[j] += vz[j]*w1 + vy[j]*w0, a[j] += vy[j]*w1 - vz[j]*w0; }
...
static void mp3d_synth_granule(float *qmf_state, float *grbuf, int nbands, int nch, mp3d_sample_t *pcm, float *lins)
{
    ...
    __builtin___memcpy_chk (lins, qmf_state, sizeof(float)*15*64, __builtin_object_size (lins, 0));
    ...
    {
        __builtin___memcpy_chk (qmf_state, lins + nbands*64, sizeof(float)*15*64, __builtin_object_size (qmf_state, 0));
    }
}
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    if (mp3_bytes > 4 && dec->header[0] == 0xff && hdr_compare(dec->header, mp3))
    {
        ...
        if (frame_size != mp3_bytes && (frame_size + 4 > mp3_bytes || !hdr_compare(mp3, mp3 + frame_size)))
        {
            frame_size = 0;
        }
    }
    if (!frame_size)
    {
        __builtin___memset_chk (dec, 0, sizeof(mp3dec_t), __builtin_object_size (dec, 0));
    ...
    hdr = mp3 + i;
    __builtin___memcpy_chk (dec->header, hdr, 4, __builtin_object_size (dec->header, 0));
    info->frame_bytes = i + frame_size;
    info->channels = (((hdr[3]) & 0xC0) == 0xC0) ? 1 : 2;
    ...
    bs_init(bs_frame, hdr + 4, frame_size - 4);
    if ((!((hdr[1]) & 1)))
    {
    ...
    if (info->layer == 3)
    {
        ...
        if (success)
        {
            for (igr = 0; igr < (((hdr[1]) & 0x8) ? 2 : 1); igr++, pcm += 576*info->channels)
            {
                __builtin___memset_chk (scratch.grbuf[0], 0, 576*2*sizeof(float), __builtin_object_size (scratch.grbuf[0], 0));
