...
static void dump_gr_info(char *label, L3_gr_info_t *gr_info) {
    printf(
        ...
        gr_info->region_count[0],
        gr_info->region_count[1],
        gr_info->region_count[2],
        gr_info->subblock_gain[0],
        gr_info->subblock_gain[1],
        gr_info->subblock_gain[2],
        gr_info->preflag,
        gr_info->scalefac_scale,
        gr_info->count1_table,
        gr_info->scfsi
    );
}
...
static uint32_t get_bits(bs_t *bs, int n)
{
    ...
    printf("get_bits: %d, %d, %d\n", cache, (next >> -shl), cache | (next >> -shl));
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    printf("   -> %d, %d, %d\n", gr_count, main_data_begin, scfsi);

    dump_gr_info("L3_read_side_info[0]", gr);
    do
    {
        if (HDR_IS_MONO(hdr))
        {
...
#define FLUSH_BITS(n) { bs_cache <<= (n); bs_sh += (n); printf("FLUSH_BITS: %d, %d, %d\n", n, bs_cache, bs_sh); }
#define CHECK_BITS    { while (bs_sh >= 0) { bs_cache |= (uint32_t)*bs_next_ptr++ << bs_sh; bs_sh -= 8; } printf("CHECK_BITS: %d, %d\n", bs_sh, bs_cache); }
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
        ...
        if (linbits)
        {
            do
            {
                ...
                do
                {
                    ...
                    CHECK_BITS;
                    printf("pairs_to_decode: %d\n", pairs_to_decode);
                } while (--pairs_to_decode);
                printf("big_val_cnt: %d, %d, %d\n", big_val_cnt, np, sfb_cnt);
    ...
    for (np = 1 - big_val_cnt;; dst += 4)
    {
        ...
        FLUSH_BITS(leaf & 7);
        printf("[bb]: bs_next_ptr - bs->buf=%d, BSPOS=%d, layer3gr_limit=%d, bs_sh=%d\n", bs_next_ptr - bs->buf, BSPOS, layer3gr_limit, bs_sh);
...
static void L3_save_reservoir(mp3dec_t *h, mp3dec_scratch_t *s)
{
    ...
    dump("h->reserv_buf", h->reserv_buf, remains);
    printf("%d, %d\n", pos, remains);
    h->reserv = remains;
}
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs, mp3dec_scratch_t *s, int main_data_begin)
{
    ...
    dump("L3_restore_reservoir.reserv_buf", h->reserv_buf + MINIMP3_MAX(0, h->reserv - main_data_begin), MINIMP3_MIN(h->reserv, main_data_begin));
    dump("L3_restore_reservoir.buf", bs->buf + bs->pos/8, frame_bytes);
    ...
    printf("L3_restore_reservoir: %d, %d, %d, %d\n", frame_bytes, bytes_have, h->reserv, main_data_begin);
    dump("L3_restore_reservoir.maindata", s->maindata, bytes_have + frame_bytes);
    printf("L3_restore_reservoir.reserv: %d, %d\n", h->reserv, main_data_begin);
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    if (info->layer == 3)
    {
        dump_gr_info("L3_read_side_info.pre", scratch.gr_info);
        ...
        if (success)
        {
            for (igr = 0; igr < (HDR_TEST_MPEG1(hdr) ? 2 : 1); igr++, pcm += 576*info->channels)
            {
                ...
                printf("mp3dec_decode_frame:\n");
                dump("scratch.grbuf[0]", scratch.grbuf[0], 576*2*sizeof(float));
